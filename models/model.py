...
class BaseModel(nn.Module):
    def __init__(self, model_name, num_classes=1000, inp_chans=3, pretrained=False, metric='linear'):
        super().__init__()
        ...
        if inp_chans != 3:
            backbone[0] = nn.Conv2d(inp_chans, 64, kernel_size=7, stride=2, padding=3, bias=False)
...
if __name__ == '__main__':
    ...
    x = torch.randn((1, model.inp_chans, 224, 224))
    out = model(x)
    print(out.size())
    print(model)